...
def find(board, word):
    ...
    for i in range(5):
        for j in range(5):
            if dfs_helper(i, j, board, word):
                return True
    return False

def dfs_helper(i, j, board, word):
    if len(word) == 0:
        return True
    if i < 0 or i > 4 or j < 0 or j > 4 or board[i][j] != word[0]:
        return False
    temp = board[i][j]
    board[i][j] = '#'
    res = False
    if dfs_helper(i + 1, j, board, word[1:]) or dfs_helper(i - 1, j, board, word[1:]) or \
        dfs_helper(i, j + 1, board, word[1:]) or dfs_helper(i, j - 1, board, word[1:]):
            res = True
    board[i][j] = temp
    return res
