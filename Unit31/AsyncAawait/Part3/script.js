...
$btn.on("click", async function () {
    ...
    let result = await Promise.all(all_urls.map(url => $.getJSON(url)));
    name_and_images = result.map(pokemon => ({
        name: pokemon.name,
        imgSrc: pokemon.sprites.front_default
    }));
    let url = await Promise.all(result.map(p => $.getJSON(p.species.url)));
    url.forEach((d, i) => {
        let descriptionObj = d.flavor_text_entries.find(function (entry) {
            return entry.language.name === "en";
        });
